...
void circle(float r, float xc, float yc, float zc)
{
	...
	for (int j = 0; j <= 360; j+=2) {
		...
		float rad = j*(3.14 / 180);

		xc = cos(rad);
		yc = sin(rad);

		glColor3f(1, 0, 0);
		for (float i = 0; i <= 360; i += 000.5)
		{
			float rad = (3.14 / 180)*i;

			float z = r * cos(rad) + zc;
			float y = r * sin(rad) + yc;
...
int main(int argc, char*argv[])
{
	...
	glutInitDisplayMode(GLUT_SINGLE| GLUT_RGB|GLUT_DEPTH); //setting the Display mode
